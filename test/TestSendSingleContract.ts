...
import fs from "fs"
import { deploy, deploySingleContract, deployTestTokenERC20 } from "../scripts/deploy";
const {
...
describe('Test Multisend single contract functionality', async function () {
    let multisendContract

    let testData: { address: string; value: number }[]
    let accounts: string[]
    let sender
    let admin
    ...
    before(async function () {
        multisendContract = await deploySingleContract()
    ...
    it('should set fixed fees only with owner or admin account', async () => {
        let tx = await multisendContract.setFixedFee(ethers.utils.parseEther(process.env.FIXEDFEES))
        await tx.wait()
        let signers = await ethers.getSigners()
        await expectRevert.unspecified(multisendContract.connect(signers[2]).setFixedFee(ethers.utils.parseEther(process.env.FIXEDFEES)))
    })
    it('should get fixed fees with all accounts', async () => {
        fixedFees = await multisendContract.getFixedFee()
    ...
    it('should send native token to multiple address with same amount', async () => {
        ...
        //send 1 eth to all
        let tx = await multisendContract.connect(sender).nativeSendSameValue(receivers, ethers.utils.parseEther("1"), { value: ethers.utils.parseEther(receivers.length.toString()).add(fixedFees) })
        tx = await tx.wait()
    ...
    it('should send native token to multiple address with different amount', async () => {
        ...
        //send 1 eth to all
        let tx = await multisendContract.connect(sender).nativeSendDifferentValue(receivers, values, { value: totalToSend.add(fixedFees) })
        tx = await tx.wait()
    ...
    it('should send erc20 token to multiple address with same amount', async () => {
        ...
        tx = await erc20.connect(sender).increaseAllowance(multisendContract.address, totalToSend);
        await tx.wait()
        tx = await multisendContract.connect(sender).sendSameValue(erc20.address, receivers, ethers.utils.parseEther("1"), { value: fixedFees })
        tx = await tx.wait()
    ...
    it('should send erc20 token to multiple address with different amount', async () => {
        ...
        tx = await erc20.connect(sender).increaseAllowance(multisendContract.address, totalToSend);
        await tx.wait()
        tx = await multisendContract.connect(sender).sendDifferentValue(erc20.address, receivers, values, { value: fixedFees })
        tx = await tx.wait()
