...
function log(logEnabled, message) {
...
async function deploy({
  ...
  log(logEnabled, "DiamondCutFacet deployed:", diamondCutFacet.address);
  ...
  log(logEnabled, "Diamond deployed:", diamond.address);
  ...
  log(logEnabled, diamondName + " deployed:", diamondInit.address);

  // deploy facets
  log(logEnabled, "");
  log(logEnabled, "Deploying facets");
  const cut = [];
  for (const FacetName of facets) {
    const Facet = await ethers.getContractFactory(FacetName);
    const facet = await Facet.deploy();
    await facet.deployed();
    log(logEnabled, `${FacetName} deployed: ${facet.address}`);
    cut.push({
  ...
  log(logEnabled, "");
  log(logEnabled, "Diamond Cut:", cut);
  ...
  log(logEnabled, "Diamond cut tx: ", tx.hash);
  receipt = await tx.wait();
  if (!receipt.status) {
    throw Error(`Diamond upgrade failed: ${tx.hash}`);
  }
  log(logEnabled, "Completed diamond cut");
